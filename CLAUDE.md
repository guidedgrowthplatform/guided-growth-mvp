...
### iOS mic permission (Info.plist)

`ios/` is gitignored and regenerated by `npx cap sync`, so privacy keys are re-injected by `scripts/patch-ios-plist.mjs` — run via `npm run cap:sync` (and `cap:sync:all`) automatically. `NSMicrophoneUsageDescription` is required or home/coach voice is dead on iOS (#88); `NSLocalNetworkUsageDescription` is required or Vapi (WebRTC ICE) silently fails to connect. After any manual `npx cap sync ios`, re-run `node scripts/patch-ios-plist.mjs`.

### Screen wake-lock during voice sessions (#208)

`@capacitor-community/keep-awake` added — run `npx cap sync` before the next native build (`ios/`/`android/` are gitignored). `src/lib/services/keepAwake.ts` holds the lock while `owner.kind !== 'idle'` (native plugin / web Screen Wake Lock), driven from `VoiceContext`. Web sentinel auto-releases on tab hide; the GLOB-03 visibilitychange handler in `src/App.tsx` reacquires on return-to-visible and suspends on background for battery.
