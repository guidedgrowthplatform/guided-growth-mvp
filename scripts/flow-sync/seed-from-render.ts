...
function loadRenderBeats(path: string): RenderBeat[] {
  ...
  if (!Array.isArray(beats) || beats.length < 20) {
    throw new Error(
      'BEATS eval produced ' + (beats as unknown[])?.length + ' entries; expected the full flow',
    );
  }
  return beats;
}
...
const FORBIDDEN_IN_SPEECH =
  /\b(tap|taps|tapping|scroll|scrolls|scrolling|click|clicks|clicking|press|presses|pressing|swipe|swipes|swiping)\b/i;

function lintSpoken(where: string, text: string | undefined): void {
  if (!text) return;
  if (/[–—]/.test(text))
    problems.push('EM/EN DASH in spoken line at ' + where + ': ' + text.slice(0, 60));
  const hit = text.match(FORBIDDEN_IN_SPEECH);
  if (hit)
    problems.push(
      'FORBIDDEN word "' + hit[0] + '" in spoken line at ' + where + ': ' + text.slice(0, 80),
    );
}
...
function main(): void {
  ...
  const clipsManifest = new Set(
    readFileSync(arg('clips-manifest'), 'utf8')
      .split('\n')
      .map((l) => l.trim())
      .filter(Boolean)
  ...
  function buildNarration(m: MetaBeat, rb: RenderBeat, where: string): NarrationSeg[] | undefined {
    ...
    const closeLines = new Set(
      [
        (rb.props as Dict | undefined)?.closeCoachLine,
        (rb.props as Dict | undefined)?.confirmCoachLine,
      ]
    ...
    return m.narration.map((seg, i) => {
      ...
      const kind: NarrationSeg['kind'] = isClose
        ? 'close'
        : seg.bubble != null
          ? 'bubble'
          : 'reveal';
      ...
      if (clipId) {
        ...
        if (!clipsManifest.has(file))
          problems.push('CLIP FILE MISSING at ' + where + '.narration[' + i + ']: ' + file);
  ...
  function buildMp3Assets(
    rb: RenderBeat,
    m: MetaBeat | undefined,
    narration: NarrationSeg[] | undefined,
    where: string,
  ): Mp3Asset[] {
    const assets: Mp3Asset[] = [];
    const seen = new Set<string>();
    const push = (a: Mp3Asset) => {
      if (!seen.has(a.id)) {
        seen.add(a.id);
        assets.push(a);
      }
    ...
    if (rb.id === 'coach-greeting') {
      ...
      push({
        id: 'splash_welcome',
        label: 'splash_welcome',
        file: '/voice/splash_welcome.mp3',
        transcript: opener,
        timing: 'full-beat',
      });
      return assets;
    ...
    const elementByClip = new Map(
      (m?.elements ?? []).filter((e) => e.clip).map((e) => [e.clip!, e]),
    );
    if (narration) {
    ...
    for (const el of m?.elements ?? []) {
      ...
      if (!clipsManifest.has(file)) {
        problems.push('CLIP FILE MISSING at ' + where + ' element ' + el.elementId + ': ' + file);
        continue;
      }
      clipIdToFile.set(clipId, file);
      push({
    ...
    if (rb.engine === 'MP3') {
      for (const key of ['coachLine', 'coachLine2', 'closeCoachLine', 'confirmCoachLine']) {
        ...
        const id = file
          ...
          .replace(/\.(wav|mp3)$/, '');
        if (!clipsManifest.has(file)) {
          problems.push('CLIP FILE MISSING at ' + where + '.props.' + key + ': ' + file);
          continue;
        }
        clipIdToFile.set(id, file);
  ...
  function convert(rb: RenderBeat): Dict {
    ...
    const bubbleSays = (narration ?? [])
      .filter((s) => s.kind === 'bubble' && s.say)
      .map((s) => s.say!.trim());
    const propLines = [props.coachLine, props.coachLine2]
      .filter((v): v is string => typeof v === 'string')
      .map((s) => s.trim());
    ...
    if (rb.engine === 'MP3') {
      for (const line of propLines) {
        if (!voiceClips.has(line))
          problems.push(
            'LINE NOT IN voiceClips (copy drift?) at ' + where + ': ' + line.slice(0, 80),
          );
      }
    }

    const voiceEngine =
      rb.engine === 'Silent' ? ((baseMeta.voiceEngine as string) ?? 'None') : rb.engine;
    const meta: Dict = {
      ...baseMeta,
      voiceEngine,
      ...
      ...(m
        ? {
            // Silent beats carry null opener/flags in the metadata; never emit nulls
            // (the strict Export schema rejects them loud).
            ...(typeof m.opener === 'string' && m.opener ? { spokenContent: m.opener } : {}),
            ...(typeof m.openerShowsAsBubble === 'boolean'
              ? { openerShowsAsBubble: m.openerShowsAsBubble }
              : {}),
            ...(typeof m.expectedResponse === 'string' && m.expectedResponse
    ...
    // why-intro is dropped (merged into state-check), so state-check's back
    // target moves to the beat before it; without this the built-in spec's
    // backId points at a node that no longer exists and validation fails loud.
    if (rb.id === 'state-check') {
      meta.engine = { ...((meta.engine as Dict) ?? {}), backId: 'profile' };
    }
    ...
    const beat: Dict = {
      ...
      sheetStage:
        (baseBeat?.sheetStage as string) ??
        (rb.screenId ? rb.screenId + ': ' + prettyName(rb) : ''),
      ...
      ...(COMPONENT_OWNED_RENDER_IDS.has(rb.id) && FIELD_PLACEMENT.componentOwned === 'beat'
        ? { componentOwned: true }
        : {}),
      meta,
    };
    if (rb.hideOrb && FIELD_PLACEMENT.hideOrb === 'meta') (beat.meta as Dict).hideOrb = true;
    if (COMPONENT_OWNED_RENDER_IDS.has(rb.id) && FIELD_PLACEMENT.componentOwned === 'meta')
      (beat.meta as Dict).componentOwned = true;
    ...
    report.push(
      [
        ...
        baseBeat
          ? '- base: merged over beat ' + label
          : '- base: NEW BEAT (no base counterpart)' +
            (CUSTOM_ENTRY_ENGINE[rb.id]
              ? '; engine wiring is a PLACEHOLDER pending the Step-0 sample'
              : ''),
        oldLine && oldLine !== newLine
          ? '- copy: OLD "' + oldLine + '"\n- copy: NEW "' + newLine + '"'
          : '- copy: unchanged or new',
        narration
          ? '- narration: ' +
            narration.length +
            ' segments (' +
            narration.filter((s) => s.kind === 'bubble').length +
            ' bubbles, ' +
            narration.filter((s) => s.kind === 'reveal').length +
            ' reveals)'
          : '- narration: none (single-opener behavior)',
        mp3Assets.length
          ? '- clips: ' + mp3Assets.map((a) => a.id).join(', ')
          : '- clips: none' +
            (rb.engine === 'Cartesia' ? ' on the opener (live TTS by design)' : ''),
        baseBeat && Object.keys(baseProps).some((k) => !(rb.props ?? {})[k])
          ? '- kept base props: ' +
            Object.keys(baseProps)
              .filter((k) => !(rb.props ?? {})[k])
              .join(', ')
          : '',
        (baseBeat?.context as string)?.length
          ? '- context: kept from base (' +
            (baseBeat!.context as string).length +
            ' chars); may reference superseded copy, Sheet catch-up owns it'
          : '',
      ]
        .filter(Boolean)
        .join('\n'),
  ...
  for (const label of PRESERVED_BASE) {
    ...
    if (!b) {
      problems.push('PRESERVED base beat missing: ' + label);
      continue;
    }
    ...
    report.push(
      '### ' +
        ...
        (label === '0'
          ? 'QA launcher, engine skips it'
          : 'The Weekly day setup; not in the render, kept per plan, position flagged for conductor review') +
        ')',
    );
  }
  for (const label of DROPPED_BASE) {
    ...
    report.push(
      '### (dropped) ' +
        ...
        ' ' +
        (b?.componentType ?? '?') +
        '\n- Superseded by the render (why-intro merged into state-check). No engine node on this beat, so the graph is unaffected. sheetStage "' +
        ((b?.sheetStage as string) ?? '') +
        '" retires.',
    );
  }
  ...
  const header = [
    ...
    '- unreferenced clips: ' +
      [...clipsManifest].filter((f) => ![...clipIdToFile.values()].includes(f)).join(', '),
    '',
    problems.length
