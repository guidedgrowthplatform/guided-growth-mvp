...
const integer = (value: unknown, path: string): number => Number.isSafeInteger(value) ? value : fail(path, 'must be a safe integer');
...
function beat(value: unknown, path: string): ContractBeat {
  ...
  return {
    ...
    voice: { engine: enumValue(voice.engine, ['MP3', 'Cartesia', 'Vapi', 'Silent'], `${path}.voice.engine`), mode: voice.mode === null ? null : enumValue(voice.mode, ['Verbatim', 'Improvise'], `${path}.voice.mode`), perLine: array(voice.perLine, `${path}.voice.perLine`).map((item, i) => { const line = object(item, `${path}.voice.perLine[${i}]`); exact(line, ['seq', 'resolution', 'liveAllowed'], `${path}.voice.perLine[${i}]`); return { seq: integer(line.seq, `${path}.voice.perLine[${i}].seq`), resolution: enumValue(line.resolution, ['recorded', 'live', 'silent'], `${path}.voice.perLine[${i}].resolution`), liveAllowed: boolean(line.liveAllowed, `${path}.voice.perLine[${i}].liveAllowed`) }; }) },
...
function validateContract(value: unknown): OnboardingContractV1 {
  ...
  return {
    schemaId: enumValue(v.schemaId, ['guided-growth.onboarding-contract'], '$.schemaId'), schemaVersion: enumValue(v.schemaVersion, [1], '$.schemaVersion'), sourceGitSha: string(v.sourceGitSha, '$.sourceGitSha'), contractRevision: string(v.contractRevision, '$.contractRevision'), globalContext: string(v.globalContext, '$.globalContext'),
