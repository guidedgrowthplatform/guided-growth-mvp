...
- **`scripts/patch-ios-plist.mjs`** — `CFBundleURLName` now uses a validated `APP_IDENTIFIER` (regex `^[A-Za-z0-9.-]+$`, falls back to the prod id) instead of a hardcoded value. This field is cosmetic; the functional OAuth scheme (`guidedgrowth://`) is unchanged.
