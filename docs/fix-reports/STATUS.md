...
| B4 | MP3 clips don't play; dead air + long LLM think after path answer | 1 | **verification FAILED — fix insufficient, needs rework** | [!397](https://gitlab.com/guidedgrowth-group/guided-growth-mvp/-/merge_requests/397) | On the !397 preview every useBeatOpenerMp3 beat activates twice (~same ms); activation #1's cleanup pause()s the SHARED pooled element and its late AbortError settle()s activation #2 → every MP3 opener marked done with 0 'playing' events; say-only beats self-advance silently, hands-off, autoplay allowed AND blocked, authed + preview routes. The NotAllowed defer itself works (proven on the Cartesia opener). Fix hint: per-activation settle token; AbortError must re-arm, not settle. Post-fork dead-air facet: see 2026-07-02 20:45 worklog (fork answer persists, client never advances). |
...
| B14 | First spoken word cut off at clip start | 1 | blocked — unverifiable until B4 race fixed (clips never start on preview) | [!397](https://gitlab.com/guidedgrowth-group/guided-growth-mvp/-/merge_requests/397) | first play gated on canplaythrough (2.5s bound); USER-first-word STT variant lives on feat/onboarding-voice-track1 → Loop 5 |
...
| 1 | B3 B4 B14 B15 | preview verification DONE 2026-07-02: B3+B15 PASS, B4 FAIL (double-activation AbortError settle — needs rework), B14 blocked. !397 NOT ready to undraft (also still carries temp diagnostics dae9bf1c) | bugfix-loop1-audio | [!397](https://gitlab.com/guidedgrowth-group/guided-growth-mvp/-/merge_requests/397) |
