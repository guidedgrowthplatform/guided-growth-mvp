...
// Slug + dayIndex → completion status / state dims / gap indicator.
// No I/O; no Math.random(); fully deterministic — vitest-safe.

export type CompletionStatus = 'done' | 'missed' | null; // null = gap day (no row)
...
// Uses a djb2-based string fold into mulberry32 for uniform distribution
// over small structured key sets (verified: consistent slugs ≥85%).
...
export function isGapDay(slug: string, dayIndex: number): boolean {
  if (slug === '3w-gaps') return [3, 10, 17].includes(dayIndex);
  // 1mo-gaps: last 2 of each 7-day cycle simulate weekend gaps
...
export function completionStatus(
  ...
  switch (slug) {
    ...
    case '1mo-strongstart':
      // strong first 14 days then decay
...
export function dayStates(
  ...
  // Energy tracks previous night's sleep (lagged correlation)
  const energyBase =
    prevSleep !== null
