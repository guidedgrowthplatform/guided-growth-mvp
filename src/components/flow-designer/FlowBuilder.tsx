...
export function FlowBuilder() {
  ...
  const [mode, setMode] = useState<'flows' | 'orb' | 'app'>(() => {
    // Deep link: ...?orb opens the Orb builder, ...?app opens the App preview.
    if (typeof window !== 'undefined') {
      try {
        const q = new URLSearchParams(window.location.search);
        if (q.has('app')) return 'app';
  ...
  // App preview: the 4 features wired into one live app with the canonical orb +
  // nav, driven by the same tuner controls.
  if (mode === 'app') {
    return (
      <div
        className="flex min-h-screen flex-col items-center gap-6 p-5"
