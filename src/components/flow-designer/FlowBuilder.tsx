...
import { OrbStates } from './orb/OrbStates';
...
export function FlowBuilder() {
  ...
  const [mode, setMode] = useState<'flows' | 'orb' | 'orbstates'>(() => {
    // Deep links: ...?orbstates opens the Orb states page, ...?orb the Orb builder.
    if (typeof window !== 'undefined') {
      try {
        const q = new URLSearchParams(window.location.search);
        if (q.has('orbstates')) return 'orbstates';
        if (q.has('orb')) return 'orb';
      } catch {
        /* ignore */
      }
    }
    if (typeof localStorage === 'undefined') return 'flows';
    const m = localStorage.getItem(`${STORAGE_BASE}:mode`);
    return m === 'orb' || m === 'orbstates' ? m : 'flows';
  });
  ...
  const modeBar = (
    <div className="flex w-[400px] max-w-full items-center gap-0.5 rounded-xl border border-border bg-surface p-0.5 shadow-sm">
      {(['flows', 'orb', 'orbstates'] as const).map((m) => (
        <button
          key={m}
          type="button"
          onClick={() => setMode(m)}
        ...
        >
          {m === 'flows' ? 'Flows' : m === 'orb' ? 'Orb builder' : 'Orb states'}
        </button>
      ))}
    </div>
  );
  ...
  // Orb states: the four app orb states on one screen, each with its full
  // control stack and copyable values. Reached via ?orbstates or the toggle.
  if (mode === 'orbstates') {
    return (
      <div
        className="flex min-h-screen flex-col items-center gap-6 p-5"
