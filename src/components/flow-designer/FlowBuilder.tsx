...
// render + builder now share the VoiceMode vocabulary (Verbatim/Generative); no remap.
const RENDER_MODE_TO_BUILDER: Record<string, string> = {};
...
function withBeatMeta(
  ...
  const seed = withRenderFacts(
    sid,
    sid ? (BEAT_METADATA[sid] as BeatContextMeta | undefined) : undefined,
  );
  if (!seed) return meta;
  return { ...seed, ...(meta ?? {}) };
}
