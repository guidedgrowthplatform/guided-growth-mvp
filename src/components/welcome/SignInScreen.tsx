import { IconChatVoice, IconMic } from '@/components/icons';
import { Button } from '@/components/ui/Button';
import { DualButton } from '@/components/ui/DualButton';
import { USER_BG } from '@/components/welcome/beatMood';
import { ORB_REST_SCALE, ORB_REST_TOP } from '@/components/welcome/SplashIntro';

interface SignInScreenProps {
  onApple?: () => void;
  onGoogle?: () => void;
  onSignUp?: () => void;
  /** Heading above the auth controls. */
  heading?: string;
  /** Small line under the heading. */
  subheading?: string;
  /**
   * Show the resting orb at the bottom. On by default so this beat picks up
   * exactly where the intro animation's orb settled (shared element).
   */
  showOrb?: boolean;
}

// Beat 2: the sign-in screen. Standalone so it is not trapped inside the flow
// builder and can be sequenced after the intro animation beat.
export function SignInScreen({
  onApple,
  onGoogle,
  onSignUp,
  heading = 'Your coach is ready',
  subheading = 'Create an account to begin',
  showOrb = true,
}: SignInScreenProps) {
  return (
    <div
      className="relative flex h-full w-full flex-col items-center overflow-hidden bg-white"
      style={{ backgroundImage: USER_BG }}
    >
      {/* Heading block, near the top */}
      <div
        style={{
          position: 'absolute',
          top: '15%',
          left: 0,
          right: 0,
          padding: '0 28px',
          textAlign: 'center',
        }}
      >
        <h1
          style={{
            fontFamily: 'Urbanist, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
            fontSize: 'clamp(24px, 6.5vw, 30px)',
            fontWeight: 700,
            letterSpacing: '-0.02em',
            color: 'rgb(19, 91, 235)',
            margin: 0,
            lineHeight: 1.15,
          }}
        >
          {heading}
        </h1>
        {subheading && (
          <p
            style={{
              fontFamily: 'Urbanist, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
              fontSize: '15px',
              fontWeight: 500,
              color: 'rgb(100,116,139)',
              margin: '8px 0 0',
              letterSpacing: '0.01em',
            }}
          >
            {subheading}
          </p>
        )}
      </div>

      {/* Auth controls, in the lower middle, above the resting orb */}
      <div
        style={{
          position: 'absolute',
          bottom: 175,
          left: 0,
          right: 0,
          padding: '0 28px',
          display: 'flex',
          flexDirection: 'column',
          gap: '12px',
        }}
      >
        <Button variant="social-dark" size="auth" fullWidth onClick={onApple}>
          <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.8 1.18-.24 2.31-.93 3.57-.84 1.51.12 2.65.72 3.4 1.8-3.12 1.87-2.38 5.98.48 7.13-.57 1.5-1.31 2.99-2.54 4.09zM12.03 7.25c-.15-2.23 1.66-4.07 3.74-4.25.29 2.58-2.34 4.5-3.74 4.25z" />
          </svg>
          Continue with Apple
        </Button>

        <Button variant="social-light" size="auth" fullWidth onClick={onGoogle}>
          <svg className="h-5 w-5" viewBox="0 0 24 24" aria-hidden="true">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.96 10.96 0 0 0 1 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          Continue with Google
        </Button>

        <p
          style={{
            fontFamily: 'Urbanist, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif',
            fontSize: '14px',
            fontWeight: 500,
            color: 'rgb(100,116,139)',
            textAlign: 'center',
            margin: '4px 0 0',
            letterSpacing: '0.01em',
            userSelect: 'none',
          }}
        >
          No account yet?{' '}
          <span
            onClick={onSignUp}
            style={{ color: 'rgb(19,91,235)', fontWeight: 600, cursor: 'pointer' }}
          >
            Sign up free
          </span>
        </p>
      </div>

      {/* Resting orb at the bottom, matching the intro animation's settled pose
          so the orb reads as the same element carried over from beat 1. */}
      {showOrb && (
        <div
          style={{
            position: 'absolute',
            left: '50%',
            top: ORB_REST_TOP,
            transform: `translate(-50%, -50%) scale(${ORB_REST_SCALE})`,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            pointerEvents: 'none',
          }}
        >
          <DualButton
            size={150}
            leftActive
            rightActive
            activeRings={null}
            ringCount={3}
            ringStep={7}
            leftIcon={<IconChatVoice size={38} />}
            rightIcon={<IconMic size={36} />}
            leftAriaLabel="Coach voice"
            rightAriaLabel="Microphone"
          />
        </div>
      )}
    </div>
  );
}
