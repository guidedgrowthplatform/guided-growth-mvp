...
export function AddHabitModal({ open, onClose, onAdd }: AddHabitModalProps) {
  const { register, handleSubmit, reset, formState: { errors } } = useForm<MetricCreateForm>({
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Add New Habit">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <Input label="Question (optional)" {...register('question')} error={errors.question?.message} />
        <div className="grid grid-cols-2 gap-4">
          <Select label="Input Type" {...register('input_type')} options={INPUT_TYPES} error={errors.input_type?.message} />
          <Select label="Frequency" {...register('frequency')} options={FREQUENCIES} error={errors.frequency?.message} />
        </div>
