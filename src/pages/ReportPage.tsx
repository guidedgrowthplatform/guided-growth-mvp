  ...
  return (
    <div>
      ...
      {activeMetrics.length > 0 && streaks.some((s) => s.current > 0 || s.longest > 0) && (
        <div className="glass rounded-2xl shadow-xl border border-cyan-200/50 mt-6 p-6">
          <h2 className="text-xl font-semibold text-slate-800 mb-4">Streaks</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {streaks.filter((s) => s.current > 0 || s.longest > 0).map(({ metric, current, longest }) => (
              <div key={metric.id} className="flex items-center gap-3 p-3 rounded-xl bg-white/50">
                <div className="text-2xl">{current > 0 ? '\uD83D\uDD25' : '\u2B50'}</div>
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm text-slate-800 truncate">{metric.name}</div>
                  <div className="text-xs text-slate-500">
                    Current: <span className="font-semibold text-cyan-600">{current} day{current !== 1 ? 's' : ''}</span>
                    {longest > current && (
                      <span className="ml-2">Best: <span className="font-semibold text-amber-600">{longest}</span></span>
                    )}
                  </div>
                </div>
              </div>
