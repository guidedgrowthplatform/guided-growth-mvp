...
export const WEEKLY_TOOLS: readonly WeeklyTool[] = [
  {
    ...
    screen: 'WCHECK-EDIT',
    description:
      'Edit an existing habit during The Weekly. Renaming covers the "shrink it" move ' +
      '(e.g. rename "Read a chapter" to "Read a page" to lower the bar). Changing ' +
      'frequency or schedule_days covers the "lower the frequency" move (e.g. five days ' +
      'a week becomes three). Only call once the user has actually agreed to the change. ' +
      'Provide the habit `name` (case-insensitive, matches the current name) and ONLY the ' +
      'field(s) that are changing — unspecified fields are preserved. Fails if the habit ' +
    ...
    parameters: {
      ...
      properties: {
        ...
        new_name: {
          type: 'string',
          description:
            'New name, if renaming (the shrink-it move, e.g. "Read a chapter" -> "Read a page"). 1-100 chars.',
        },
        frequency: {
          type: 'string',
  ...
  },
  {
    ...
    screen: 'WCHECK-EDIT',
    description:
      'The "drop it" move: archive a habit that is not serving the user or is not the ' +
      'priority right now. This is a SOFT delete (is_active = false) — habit history stays, ' +
      'nothing is hard-deleted. Only call once the user has actually agreed to drop the ' +
