...
export async function markRest(
  ...
  for (const raw of rawDates) {
    ...
    if (date > today)
      return invalid(`Cannot mark a rest for "${habit.name}" on a future date (${date}).`);
    await pool.query(
      `INSERT INTO habit_completions (anon_id, habit_id, date, status)
       VALUES ($1, $2, $3, 'rest')
       ON CONFLICT (habit_id, date) DO UPDATE SET completed_at = now(), status = 'rest'`,
