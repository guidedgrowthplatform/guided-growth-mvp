...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (route === 'entries') {
    ...
    // POST /api/reflections/entries/upload — upload image
    if (journalSub === 'upload') {
      if (req.method !== 'POST') {
    ...
    // POST /api/reflections/entries — create entry
    ...
    // POST /api/reflections/entries/:id/insight — lazy-backfill AI insight
    ...
    // GET/PUT/DELETE /api/reflections/entries/:id
    if (journalSub) {
      const entryId = validateUUID(journalSub);
      if (!entryId) return res.status(400).json({ error: 'Invalid entry ID' });
      ...
      if (req.method === 'GET') {
        ...
        if (result.rows.length === 0) {
          console.warn('[reflections.get] 404', { entryId, requestUserId: user.id });
