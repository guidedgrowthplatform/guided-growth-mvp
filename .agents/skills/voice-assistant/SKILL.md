...
- **Editing tools** (e.g. adding a new tool call) → both repos: add the function in `tools.py`, register it in `main.py`'s `tools=[…]` list, and make sure the LLM is told _when_ to call it via the system prompt or per-screen JSON.
