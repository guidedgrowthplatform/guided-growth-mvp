...
If the UI needs any of those, route through Supabase (writes by tools → Realtime → browser) or build a custom event over the `custom` channel and read it in the agent. The current MVP only needs tool _side effects_, which Supabase Realtime delivers.
